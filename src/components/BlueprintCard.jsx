...
const BlueprintCard = ({ blueprint, status = 'unknown', onClick, isOverview = false, users = [], filteredUser = null, overviewMode = 'missing', individualMode = 'missing', showName = false }) => {
    ...
    if (isOverview && users.length > 0) {
        ...
        if (visibleUsers.length === 1) {
            overviewGradient = visibleUsers[0].color;
    ...
    const getIndividualBorderStyle = () => {
        if (isOverview) return 'border-2 border-transparent';

        const isOwned = status === 'owned';
        const isMissing = !isOwned;

        if (individualMode === 'missing' && isMissing) {
            return 'border-2 border-arc-red drop-shadow-[0_0_5px_rgba(255,68,68,0.5)]';
        }
        if (individualMode === 'owned' && isOwned) {
            return 'border-2 border-arc-green drop-shadow-[0_0_5px_rgba(68,255,68,0.5)]';
        }

        return 'border-2 border-[#333] hover:border-gray-500';
    };
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          `}
            ...
            >
                ...
                {!isOverview && (
                    <div className={`absolute top-1 right-1 w-2 h-2 rounded-full z-10 ${individualMode === 'missing' && status !== 'owned' ? 'bg-arc-red' :
                            individualMode === 'owned' && status === 'owned' ? 'bg-arc-green' :
                                'hidden'
                        }`} />
                )}
            </div>
            {showName && !blueprint.isBlank && (
                <span className="text-xs text-gray-400 text-center max-w-[100px] truncate">
                    {blueprint.name}
                </span>
            )}
        </div>
